      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {data.services?.map((s: ServiceHighlightsContent["services"][number], idx: number) => (
            <div key={idx} className="group rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-transform hover:-translate-y-1">
              ...
              <div className="mt-4 h-1 w-full overflow-hidden rounded-full bg-slate-100">
                <div className="h-1" style={{ width: "100%", background: `linear-gradient(90deg, ${primary}, ${secondary})` }} />
              </div>
            </div>
          ))}
